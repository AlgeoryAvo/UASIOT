...
class Temp_Measurement
{
    ...
    function read_data()
    {
        $sql_query = "SELECT * FROM dht_data";
        echo $this->execute_query($sql_query, [], true);
    }

    function create_data($temp, $humid)
    {
        $sql_query = "INSERT INTO dht_data (temperature_data, humidity_data) VALUES ('".$temp."','".$humid."')";
        echo $this->execute_query($sql_query);
    }

    function update_data($id, $update_data = [])
    {
        $sql_query = "UPDATE dht_data SET ".$update_data['column']." = ".$update_data['value']." WHERE id = ".$id."";
        $this->execute_query($sql_query);
    }

    function delete_data($id)
    {
        $sql_query = "DELETE FROM dht_data WHERE id = ".$id."";
        echo $this->execute_query($sql_query);
    }
